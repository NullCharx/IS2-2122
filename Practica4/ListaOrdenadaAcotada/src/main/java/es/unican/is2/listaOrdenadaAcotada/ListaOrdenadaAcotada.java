...
public class ListaOrdenadaAcotada<E extends Comparable<E>> implements IListaOrdenadaAcotada<E> {
	...
	//Modifica para que lance un error si el indice esta por encima del ultimo elemento lleno
	public E get(int indice) {
		//Comruebo si exite un elemento en el indice indicado (Implementacion corregida)
		if (indice > ultimo) {
	...
	public E remove(int indice) {
		//Comruebo si exite un elemento a borrar (lista vacia.) implementacion corregida
		if (ultimo == - 1) {
			throw new IllegalStateException();
		}
		if (indice > ultimo) {
